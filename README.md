...
- **Line coverage is approximate.** Coverage points are chosen by the Zig compiler, and it places them where *fuzzing* benefits rather than at every branch — `Sema.zig` marks error-handling branches as points of interest and explicitly declines others ("code coverage is not valuable on either branch", "not wanted for panic branches"). zig-cov takes that exact table and attributes each source line to the last point at or before it, which is exact for straight-line code.

  Conditional bodies are handled separately, because a body with no point of its own would otherwise inherit the status of the code before it and report a never-taken branch as **covered** — the error that matters, since it hides untested code. The compiler puts a counter on a body's opening line that counts *entries into that body*, so zig-cov reads that as the verdict: zero means the loop never iterated or the branch was never taken, whatever the surrounding code did. Never-executed `if`, `else`, `while` and `for` bodies are reported as misses, including the awkward case of a body ending in `return`, where the point that resumes afterwards is attributed to the body's last line. The integration test pins both.

  One gap remains, and it understates rather than overstates: lines following a `try` can be reported missed. The error branch *is* instrumented, and when it never fires it still sits between the executed code and the lines after it, which have no point of their own. A function whose every line runs but which never fails therefore reads below 100%.

  Treat line percentages as a close estimate and read the annotated source rather than a single number — **block coverage** (below) involves no inference at all. Closing the last gap needs Zig to emit a coverage point per branch; it is not LLVM's `-fsanitize-coverage` pruning, since Zig sets `PCTable` and `Inline8bitCounters` to false and emits its own instrumentation.
